...
Check out the code from Github::
    ...
    git clone https://github.com/kyuupichan/bitgesellx.git
    cd bitgesellx
...
under one account which I have called ``bitgesellx``.

Next create a directory where the database will be stored and make it
writeable by the ``bitgesellx`` account.  I recommend this directory
live on an SSD::

    mkdir /path/to/db_directory
    chown bitgesellx /path/to/db_directory
...
:file:`.service` file (see `contrib/systemd/bitgesellx.service`_).
...
logging process.  You can run :command:`svscan` under the *bitgesellx*
...
of::
    ...
    mkdir ~/service      # As bitgesellx if running svscan as that a/c
...
Suppose this directory is called :file:`scripts`, you might do::
    ...
    mkdir -p ~/scripts/bitgesellx

Then copy the all sample scripts from the ElectrumX source tree there::

    cp -R /path/to/repo/bitgesellx/contrib/daemontools ~/scripts/bitgesellx
...
svscan is now waiting for services to be added to the directory::
    ...
    cd ~/service
    ln -s ~/scripts/bitgesellx bitgesellx
...
`contrib/raspberrypi3/install_bitgesellx.sh`_.

See also `contrib/raspberrypi3/run_bitgesellx.sh`_ for an easy way to
configure and launch bitgesellx.
...
Once the process has terminated, you can start it up again with::
    ...
    svc -u ~/service/bitgesellx

You can see the status of a running service with::

    svstat ~/service/bitgesellx
...
These instructions are based on those of the ``bitgesell-server``
documentation.
...
challenge password just leave it empty and press enter::
    ...
    Common Name (eg, YOUR name) []: bitgesell-server.tld
    ...
...
You may choose to run bitgesellx on a different port than 50001
...
.. _`contrib/systemd/bitgesellx.service`: https://github.com/kyuupichan/bitgesellx/blob/master/contrib/systemd/bitgesellx.service
...
.. _`contrib/raspberrypi3/install_bitgesellx.sh`: https://github.com/kyuupichan/bitgesellx/blob/master/contrib/raspberrypi3/install_bitgesellx.sh
.. _`contrib/raspberrypi3/run_bitgesellx.sh`: https://github.com/kyuupichan/bitgesellx/blob/master/contrib/raspberrypi3/run_bitgesellx.sh
