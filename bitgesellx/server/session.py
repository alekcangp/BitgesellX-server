...
import bitgesellx
from bitgesellx.lib.merkle import MerkleCache
from bitgesellx.lib.text import sessions_lines
import bitgesellx.lib.util as util
from bitgesellx.lib.hash import (sha256, hash_to_hex_str, hex_str_to_hash,
                                HASHX_LEN, Base58Error)
from bitgesellx.server.daemon import DaemonError
from bitgesellx.server.peers import PeerManager
...
class SessionManager:
    ...
    def __init__(self, env, db, bp, daemon, mempool, shutdown_event):
        ...
        self._sslc = None
        # Event triggered when bitgesellx is listening for incoming requests.
    ...
    async def rpc_add_peer(self, real_name):
        ...
        real_name: "bgl.bitgesellx.com t50001 s50002" for example
        '''
...
class ElectrumX(SessionBase):
    ...
    def server_features(cls, env):
        ...
        return {
            'hosts': hosts_dict,
            'pruning': None,
            'server_version': bitgesellx.version,
    ...
    def server_version_args(cls):
        ...
        return [bitgesellx.version, cls.protocol_min_max_strings()]
    ...
    async def address_status(self, hashX):
        ...
        '''
        # Note history is ordered and mempool unordered in bitgesell-server
    ...
    async def unconfirmed_history(self, hashX):
        # Note unconfirmed history is unordered in bitgesell-server
    ...
    async def replaced_banner(self, banner):
        ...
        for pair in [
                ('$SERVER_VERSION', bitgesellx.version_short),
                ('$SERVER_SUBVERSION', bitgesellx.version),
    ...
    async def banner(self):
        ...
        banner = f'You are connected to an {bitgesellx.version} server.'
        self.bump_cost(0.5)
    ...
    async def server_version(self, client_name='', protocol_version=None):
        ...
        return (bitgesellx.version, self.protocol_version_string())
    ...
    async def crash_old_client(self, ptuple, crash_client_ver):
        if crash_client_ver:
            ...
            if is_old_protocol and is_old_client:
                ...
                # this can crash bitgesell client 2.6 <= v < 3.1.2
                await self.send_notification('blockchain.relayfee', ())
                # this can crash bitgesell client (v < 2.8.2) UNION (3.0.0 <= v < 3.3.0)
...
class BitgesellX(DashElectrumX):
    '''A TCP server that handles incoming Electrum-SMART connections.'''
